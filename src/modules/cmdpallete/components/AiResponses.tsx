...
const AiResponses = ({ response, isAiFinished, onAiFinished, onAiClose }: AiResponsesProps) => {
  return (
    <>
      {response ? (
        response?.includes('```') ? (
        ...
        <Typewriter
          onInit={(typewriter) => {
            typewriter
              .typeString('Oops! The AI seems to be lost. \u00A0 😵‍💫 \u00A0\u00A0')
              .pauseFor(1000)
              .typeString('<br/><br/>')
              .typeString(
      ...
      {isAiFinished && (
        <div className='flex justify-center mt-6 transition-all duration-300'>
          <Button onClick={onAiClose} data-umami-event='Click Back from AI Response'>
            <BackButton />
            Back
          </Button>
        </div>
