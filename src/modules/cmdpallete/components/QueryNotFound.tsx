...
const QueryNotFound = ({ query, onAskAiAssistant, onFindGoogle, handleDocsearchSelect }: QueryNotFoundProps) => {
  const { data = [], isLoading } = useSWR(['docsearch', query], ([_, key]) => getDocsearchHits(key), {
    isPaused: () => !docSearch.appId,
        ...
        <div className='flex flex-col pt-5 pb-10 px-5 space-y-6 items-center'>
          <div className='text-neutral-500 text-center space-y-2'>
            <p>
              ...
              <span className='italic text-neutral-600 dark:text-neutral-400 ml-1 mr-2'>`{query}`</span>
              in this website.
            </p>
            <p className='text-neutral-600 dark:text-neutral-400'>
              {featureSwich.ai ? 'Ask my AI Assistant or find in Google instead?' : 'Find in Google instead?'}
            </p>
          </div>
          ...
          <p className='text-neutral-500 text-sm'>Press `ESC` to close this window</p>
        </div>
      )}
      {!isLoading && data.length > 0 && (
        <Combobox onChange={(url: string) => handleDocsearchSelect(url)} as='div' className='w-full'>
                    ...
                    >
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: `${hit.hierarchy.lvl0 ?? ''} ${hit.hierarchy.lvl1 ? `> ${hit.hierarchy.lvl1}` : ''} ${
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: hit.content || hit.hierarchy.lvl1 || hit.hierarchy.lvl2 || hit.hierarchy.lvl0,
                        }}
                      ></div>
                    </div>
                  )}
