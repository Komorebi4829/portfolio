  ...
  return (
    <>
      <div className='flex flex-auto bg-neutral-900 border border-neutral-700 rounded-t-md'>
        ...
        >
          <Panel ref={editorRef} defaultSize={50} minSize={20} collapsible={true}>
            <PanelHeader title='JavaScript'>
              <div className='flex items-center gap-5'>
                <div className='cursor-pointer' onClick={handleClearCode} data-umami-event='Clear Editor Playground'>
                  <ClearIcon size={18} className={clsx('text-neutral-400', code && 'text-red-400')} />
                </div>
                <div className='cursor-pointer' onClick={onRunCode} data-umami-event='Run Code Playground'>
                  <PlayIcon size={18} className={clsx('text-sky-500', !code && '!text-neutral-400')} />
                </div>
              </div>
            </PanelHeader>
            <CodeEditor
              ...
              onChange={(newCode) => newCode !== undefined && onSetCode(newCode)}
            />
          </Panel>
          ...
          <Panel defaultSize={50} minSize={20} collapsible={true}>
            <PanelHeader title='Console'>
              <div className='flex items-center'>
                <div className='cursor-pointer' onClick={handleClearOutput} data-umami-event='Clear Output Playground'>
                  <ClearIcon size={18} className={clsx('text-neutral-400', output && 'text-red-400')} />
                </div>
              </div>
            </PanelHeader>
            <ConsoleOutput output={output} isError={isError} isFullScreen={isFullScreen} />
          </Panel>
        </PanelGroup>
      </div>
