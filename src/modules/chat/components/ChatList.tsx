...
const ChatList = ({ messages, isWidget = false, onDeleteMessage }: ChatListPropsNew) => {
  ...
  return (
    <div className='rounded-lg px-1'>
      <div ref={chatListRef} className='overflow-y-auto space-y-5 py-4' style={{ height: chatListHeight }}>
        {messages?.map((chat, index) => <ChatItem key={index} onDelete={onDeleteMessage} {...chat} />)}
      </div>
    </div>
  )
}
