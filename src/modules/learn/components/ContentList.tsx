...
import { ChapterGroupProps, ContentProps, MdxFileContentProps } from '@/common/types/learn'
...
const ContentList = ({ sortedSubContents, content, title }: ContentListProps) => {
  const contentSlug: string = content?.slug ?? ''

  const groupedContent: Record<string, ChapterGroupProps> = groupContentByChapter(sortedSubContents)
    ...
    <>
      {sortedSubContents?.length > 0 ? (
        <div className='space-y-4'>
          {Object.entries(groupedContent).map(([chapterId, { chapter_title, contents }], key) => (
            <motion.div
              key={chapterId}
