...
import { utcToZonedTime, zonedTimeToUtc } from '@/common/helpers'
...
const CodingActive = ({ lastUpdate }: CodingActiveProps) => {
  ...
  useEffect(() => {
    const formatLastUpdate = (): void => {
      ...
      if (lastUpdateDate) {
        const zonedDate = utcToZonedTime(zonedTimeToUtc(lastUpdateDate, author.timeZone), author.timeZone)
  ...
  return (
    <section className='flex flex-col gap-y-2'>
      ...
      <SectionSubHeading>
        ...
        <div className='text-sm text-neutral-600 dark:text-neutral-500'>Last update: {renderLastUpdate()}</div>
      </SectionSubHeading>
