...
const sumTotalFromArray = <T extends { hours: number; minutes: number }>(data: T[], key: keyof T) => {
  return data.reduce((previousValue, currentValue) => previousValue + (currentValue[key] as number), 0) ?? 0
}

const CodingActiveList = ({ data }: CodingActiveListProps) => {
  const getLanguagesTotalHours = sumTotalFromArray<ItemProps>(data?.languages || [], 'hours')
  const getLanguagesTotalMinutes = sumTotalFromArray<ItemProps>(data?.languages || [], 'minutes')
  ...
  const getEditorTotalHours = sumTotalFromArray<ItemProps>(data?.categories || [], 'hours')
  const getEditorTotalMinutes = sumTotalFromArray<ItemProps>(data?.categories || [], 'minutes')
