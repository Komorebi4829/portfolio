...
export const _loadMdxFiles = (dirPath: string): MdxFileProps[] => {
  ...
  const contents = (files as string[])
    ...
    .map((file) => {
      ...
      const filename = file.endsWith('.mdx') ? file.replace('.mdx', '') : file.replace('.md', '')

      if (!data.featured_image_url) {
        const randomSrc = `/images/bg/bg${Math.floor(Math.random() * 15) + 1}.svg`
...
export const getCollection = (endpointer: string, withContent = false): MdxFileProps[] => {
  const dirPath = path.join(process.cwd(), 'src/contents', endpointer)
  const files = _loadMdxFiles(dirPath)

  files.sort((a, b) => {
    return getTimestamp(a.frontMatter.date as string) < getTimestamp(b.frontMatter.date as string) ? 1 : -1
  })
...
export const getEntry = (endpointer: string, slug: string): MdxFileProps | null => {
  const dirPath = path.join(process.cwd(), 'src/contents', endpointer)
