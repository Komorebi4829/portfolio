...
const getHeadingId = (children: React.ReactNode): string => {
  if (Array.isArray(children) && children.length > 0) {
    ...
    return typeof firstChild === 'string' ? firstChild : ''
  }
  return ''
}
      ...
      components={{
        a: (props) => (
          <a
            ...
            target={props.href?.startsWith(siteMetadata.siteUrl) || props.href?.startsWith('#') ? '_self' : '_blank'}
            {...props}
          />
        ),
        p: (props) => <div {...props} />,
        h2: (props) => (
          <h2 className='text-xl font-medium dark:text-neutral-300' id={getHeadingId(props.children)} {...props} />
        ),
        h3: (props) => (
          <h3
        ...
        ul: ({ ...props }) => <ul className='pl-10 space-y-3 list-disc pb-2' {...props} />,
        ol: ({ ...props }) => <ol className='pl-10 space-y-3 list-decimal pb-2' {...props} />,
